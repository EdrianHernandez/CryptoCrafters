...
// Add variables for tracking session stats
let sessionScore = 0;
let puzzlesSolved = 0;
...
// Function to encode quote with a fixed shift
...
async function checkAnswer() {
    ...
    if (isCorrect) {
        ...
        document.getElementById('result').innerHTML = `
            <div style="text-align: center; margin: 10px auto; padding: 8px 15px; background-color: #e8f5e9; border-radius: 5px; color: #2e7d32; font-weight: bold; max-width: 200px;">
                CORRECT!<br>
        ...
        document.getElementById('result').innerHTML = `
            <div style="text-align: center; margin: 10px auto; padding: 8px 15px; background-color: #ffebee; border-radius: 5px; color: #c62828; font-weight: bold; max-width: 200px;">
                TRY AGAIN!
            </div>`;
...
function provideHint() {
    ...
    if (!focusedInputField || focusedInputField.value.trim() !== "") {
        document.getElementById('result').innerHTML = `
    ...
    if (correctLetter) {
        ...
        letterBoxes.forEach(box => {
            ...
            if (encodedLetter === focusedEncodedLetter && (!input.value || input.value.trim() === "")) {
...
document.addEventListener('focusin', (event) => {
    if (event.target.tagName === 'INPUT' && event.target.parentElement.classList.contains('letter-box')) {
...
function startTimer() {
    timerInterval = setInterval(() => {
        if (timeLeft <= 0) {
            ...
            setTimeout(() => {
                displayNewQuote();
            }, 10000); // Delay before the next quote
        } else {
...
function resetTimer() {
    clearInterval(timerInterval);
    timeLeft = 480; // Reset to 8 minutes
    const minutes = Math.floor(timeLeft / 60);
    const seconds = timeLeft % 60;
    document.getElementById('timer').innerHTML = "<strong>Timer:</strong> " + String(minutes).padStart(2, '0') + ":" + String(seconds).padStart(2, '0');
    startTimer();
...
function revealAnswer() {
    // Get all letter boxes that contain inputs (only letters, not punctuation)
    const letterBoxes = Array.from(document.querySelectorAll('.letter-box')).filter(box => box.querySelector('input'));
    ...
    // Display the complete answer with author
    document.getElementById('result').innerHTML = `
        <div class="answer-reveal" style="margin: 20px 0; padding: 15px; background-color: #f0f0f0; border-radius: 5px;">
