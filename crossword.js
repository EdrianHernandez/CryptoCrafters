...
function fillInCrossWordNumbers(listitems, blockitems, blockitemsordered) {
    ...
    for (var i = 0; i < listitems.length; i++) {
        ...
        // Create the number element with class
        var element = '<span class="crossword-grid-cell-number">' + fillnumber + '</span>';
        
        // Find the parent element where the number should be placed
        var parentelement = $('#cell-position-' + coordinates[0] + '-' + coordinates[1]);

        // Ensure the parent element exists before trying to prepend
        if (parentelement.length) {
            // Prepend the number element directly to the letter cell
