    ...
    for (let word of encodedWords) {
        ...
        for (let char of word) {
            ...
            if (char.match(/[a-z]/i)) { // Only add input for letters
                ...
                // Only remove focus if we're not clicking the hint button
                input.addEventListener("blur", function(e) {
                    // Check if the related target is the hint button
                    if (e.relatedTarget && e.relatedTarget.id === 'hint-btn') {
                        // Keep the focus if clicking hint button
                        this.focus();
                        return;
                    }
                    
                    if (focusedInputField === this) {
...
async function checkAnswer() {
    ...
    if (isCorrect) {
        ...
        inputs.forEach(input => {
            if (input.value.trim() !== '') {  // Only shake filled inputs
...
function provideHint() {
    ...
    if (hintsUsed >= maxHints) {
        document.getElementById('result').innerHTML = `
            <div class="error-message" style="margin: 20px 0; padding: 15px; background-color: #ffebee; border-radius: 5px; color: #c62828;">
                <p>Hint limit reached! (${hintsUsed}/${maxHints} used)</p>
            </div>`;
        return;
    }
    ...
    // Check if the focused field already has a value
    if (focusedInputField.value.trim() !== "") {
        document.getElementById('result').innerHTML = `
            <div class="info-message" style="margin: 20px 0; padding: 15px; background-color: #e3f2fd; border-radius: 5px; color: #1565c0;">
                <p>Please select an empty letter box for the hint.</p>
            </div>`;
        return;
    }
    ...
    if (correctLetter) {
        // Increment hint counter before revealing letters
        hintsUsed++;
